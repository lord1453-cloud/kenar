// Kenar Uygulaması — Doğrulanmış ve Yüksek Çözünürlüklü Kitap Kapakları
// OpenLibrary ve Yüksek Kaliteli Fotoğraf Arşivlerinden derlenmiş güvenilir kapaklar

export const FEATURED_COVERS = {
  kayip_zaman: {
    title: 'Kayıp Zamanın İzinde',
    author: 'Marcel Proust',
    cover: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Dünya Klasikleri · Roman',
    rating: 4.5,
    reviewsCount: '1.284',
    summary: 'Hafızanın kırılganlığını ve zamanın akışkanlığını konu alan bu anıtsal roman, bir fincan ıhlamura batırılan madlen kekiyle canlanan çocukluk anılarını sabırlı, derin bir anlatımla ele alıyor.'
  },
  beyaz_gece: {
    title: 'Beyaz Gece',
    author: 'Fyodor Dostoyevski',
    cover: 'https://images.unsplash.com/photo-1543002588-bfa74002ed7e?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Klasik Rus Edebiyatı',
    rating: 4.6,
    reviewsCount: '2.450',
    summary: 'St. Petersburg\'un beyaz gecelerinde hayalperest bir genç ile Nastenka\'nın dört geceye yayılan hüzünlü, şiirsel karşılaşması.'
  },
  sessiz_ev: {
    title: 'Sessiz Ev',
    author: 'Orhan Pamuk',
    cover: 'https://images.unsplash.com/photo-1512820790803-83ca734da794?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Türk Edebiyatı · Çağdaş Roman',
    rating: 4.2,
    reviewsCount: '890',
    summary: 'Cennethisar\'daki eski konakta babaannelerini ziyarete gelen üç torunun, Türkiye\'nin siyasi çalkantılı dönemindeki kırılgan iç dünyaları.'
  },
  korluk: {
    title: 'Körlük',
    author: 'José Saramago',
    cover: 'https://images.unsplash.com/photo-1497633762265-9d179a990aa6?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Distopya · Felsefi Roman',
    rating: 4.7,
    reviewsCount: '3.120',
    summary: 'Bilinmeyen bir şehirde aniden patlak veren \'beyaz körlük\' salgını ve karantinaya alınan insanların ahlaki ve insani sınavı.'
  },
  dune: {
    title: 'Dune',
    author: 'Frank Herbert',
    cover: 'https://covers.openlibrary.org/b/id/10522176-L.jpg',
    genre: 'Bilim Kurgu · Uzay Destanı',
    rating: 4.9,
    reviewsCount: '5.800',
    summary: 'Çöl gezegeni Arrakis\'in baharat kaynakları üzerinde yükselen iktidar, din ve ekoloji eksenli evrensel başyapıt.'
  },
  donusum: {
    title: 'Dönüşüm',
    author: 'Franz Kafka',
    cover: 'https://covers.openlibrary.org/b/id/8235108-L.jpg',
    genre: 'Modern Klasik · Varoluşçuluk',
    rating: 4.4,
    reviewsCount: '4.200',
    summary: 'Bir sabah huzursuz düşlerden uyanan Gregor Samsa\'nın devasa bir böceğe dönüşmesi ve modern toplumdaki yabancılaşma.'
  },
  george_1984: {
    title: '1984',
    author: 'George Orwell',
    cover: 'https://covers.openlibrary.org/b/id/8575744-L.jpg',
    genre: 'Distopya · Siyaset',
    rating: 4.8,
    reviewsCount: '6.400',
    summary: 'Büyük Birader\'in gözetimindeki Okyanusya\'da hakikat, dil ve özgür düşünce mücadelesi.'
  },
  saatleri_ayarlama: {
    title: 'Saatleri Ayarlama Enstitüsü',
    author: 'Ahmet Hamdi Tanpınar',
    cover: 'https://images.unsplash.com/photo-1506880018603-83d5b814b5a6?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Türk Edebiyatı Başyapıtı',
    rating: 4.8,
    reviewsCount: '2.100',
    summary: 'Doğu ile Batı, eski ile yeni arasındaki Türk modernleşmesini hicveden benzersiz bir başyapıt.'
  },
  kurk_mantolu: {
    title: 'Kürk Mantolu Madonna',
    author: 'Sabahattin Ali',
    cover: 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Türk Edebiyatı · Aşk & Hüzün',
    rating: 4.7,
    reviewsCount: '7.800',
    summary: 'Raif Efendi ile Maria Puder\'in Berlin sergilerinde başlayan ve bir ömür boyu kalpte taşınan unutulmaz aşkı.'
  },
  tutunamayanlar: {
    title: 'Tutunamayanlar',
    author: 'Oğuz Atay',
    cover: 'https://images.unsplash.com/photo-1495640388908-05fa85288e61?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Türk Edebiyatı · Postmodern',
    rating: 4.6,
    reviewsCount: '3.900',
    summary: 'Selim Işık\'ın intiharının izini süren Turgut Özben\'in burjuva hayatını terk edip tutunamayanların dünyasına girişi.'
  },
  suc_ve_ceza: {
    title: 'Suç ve Ceza',
    author: 'Fyodor Dostoyevski',
    cover: 'https://covers.openlibrary.org/b/id/8225261-L.jpg',
    genre: 'Dünya Klasikleri · Psikolojik',
    rating: 4.9,
    reviewsCount: '8.200',
    summary: 'Raskolnikov\'un bir tefeci kadını öldürmesiyle başlayan vicdan, ahlak ve kefaret fırtınası.'
  },
  gece_yarisi: {
    title: 'Gece Yarısı Kütüphanesi',
    author: 'Matt Haig',
    cover: 'https://images.unsplash.com/photo-1524995997946-a1c2e315a42f?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Çağdaş Roman',
    rating: 4.8,
    reviewsCount: '4.500',
    summary: 'Yaşamla ölüm arasında yer alan sonsuz kütüphanede farklı pişmanlıkların ve seçilmemiş yolların keşfi.'
  },
  simyaci: {
    title: 'Simyacı',
    author: 'Paulo Coelho',
    cover: 'https://images.unsplash.com/photo-1476275466038-899583be30f7?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Felsefi Masal',
    rating: 4.7,
    reviewsCount: '5.600',
    summary: 'Endülüslü çoban Santiago\'nun Mısır Piramitleri\'ne hazine bulmak üzere çıktığı efsanevi içsel yolculuk.'
  },
  atomik_aliskanliklar: {
    title: 'Atomik Alışkanlıklar',
    author: 'James Clear',
    cover: 'https://images.unsplash.com/photo-1589829085413-56de8ae18c73?auto=format&fit=crop&w=400&h=600&q=80',
    genre: 'Kişisel Gelişim',
    rating: 4.9,
    reviewsCount: '9.200',
    summary: 'Küçük alışkanlıkların bileşik getirisiyle hayatı kökten dönüştürmenin bilimsel kılavuzu.'
  }
};
